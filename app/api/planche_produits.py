...
def add_product_to_planche(data: PlancheProduitCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return planche_produits_crud.create(db, data)

@router.delete("/planches/produits/{pp_id}", tags=["PlancheProduits"])
def remove_product_from_planche(pp_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
